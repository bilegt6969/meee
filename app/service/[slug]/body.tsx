...
export default async function Body({ slug }: BodyProps) {
  ...
  const { article } = await getKbArticlesByCode(slug + (currentLanguage === "MNG" ? "Data-MNG" : "Data"));
  ...
  // Sort NotHeading based on the numeric part of the code
  const sortedNotHeading = NotHeading.sort((a, b) => {
    // Match the numeric part of the code for both languages
    const matchA = a.code.match(/item-(\d+)/); // Match item-<number>
    const matchB = b.code.match(/item-(\d+)/); // Match item-<number>

    // Check if matches were found
    const numA = matchA ? parseInt(matchA[1], 10) : Infinity; // Use Infinity if no match
    const numB = matchB ? parseInt(matchB[1], 10) : Infinity; // Use Infinity if no match
  ...
  return (
    <div>
      {sortedNotHeading.map((item, index) => {
            ...
            <section key={item._id} className="mt-[-0.5rem] md:mt-[-0.5rem]">
              <div className="md:mx-2 ml-1 mt-4 md:mt-0">
                <div className="flex flex-col md:flex-row w-full">
                  ...
                  <div className="bevel6 hidden sm:block"></div> {/* This will be hidden on small screens */}
            ...
            <section key={item._id} className="mt-[-0.5rem] md:mt-[-0.5rem]">
              <div className="md:mx-2 ml-1 mt-4 md:mt-0">
                <div className="flex flex-col md:flex-row w-full">
                  ...
                  <div className="bevel6 hidden sm:block"></div>
